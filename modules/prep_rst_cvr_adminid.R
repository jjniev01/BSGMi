...
crop_raster_R <- function(srcfile, shapefile, NoDataValue) {
...
create_shape <- function(rasterFile,filePathOutputSHP) {
  ...
  ##  raster input.  
  system(
    paste(bsgm.python_path,' ',bsgm.gdal_polygonize_path,
          ' ',rasterFile,
...
create.shp.by.adminid <- function(iso3c){
  ...
  adminidlist <- bsgm.input.adminids[[iso3c]]
  ...
  str.crop <- paste(bsgm.python_path,' ',bsgm.gdal_calc_path ,' -A ',census.zones,
...
prep_rst_cvr_shp <- function(){
  ...
  for ( icountry in bsgm.input.countries ) { 
    ...
    for ( icvr in 1:length(covariates[[icountry]]) ) {
      ...
      subDir.country.shp <- as.character(bsgm.input.shp[icountry])
      
      ##  Crop the covariate raster using that shapefile while maintaining the 
      ##  No Data value from the input raster:
      crop_raster_R(raster_path,subDir.country.shp,NoDValue$NoDataValue)
    }
